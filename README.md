# Customer_dataAnalysis

## Project Overview
This project analyzes customer shopping behavior using transactional data from 3,900 purchases across multiple product categories. The objective is to uncover insights related to customer spending patterns, subscription trends, product performance, and purchasing behavior to support data-driven business decisions.

The project combines Python for data cleaning and exploratory data analysis, PostgreSQL for business query analysis, and Power BI for interactive dashboard visualization.

---

## Objectives
- Analyze customer purchase behavior and spending trends
- Identify high-performing products and categories
- Study the impact of discounts and subscriptions on revenue
- Segment customers based on purchase history
- Visualize insights using an interactive Power BI dashboard

---

## Tools & Technologies Used
- Python
- Pandas
- NumPy
- Matplotlib
- PostgreSQL
- SQL
- Power BI
- Jupyter Notebook

---

## Dataset Information
- Total Records: 3,900
- Features: 18
- Includes:
  - Customer demographics
  - Purchase details
  - Product categories
  - Review ratings
  - Subscription status
  - Shipping information
  - Discount usage

---

## Key Analysis Performed
- Revenue analysis by gender
- Top-rated products identification
- Customer segmentation
- Discount impact analysis
- Shipping type comparison
- Subscriber vs non-subscriber revenue analysis
- Revenue contribution by age group
- Repeat customer behavior analysis

---

## Project Workflow
1. Data Cleaning & Preprocessing using Python
2. Exploratory Data Analysis (EDA)
3. SQL-Based Business Analysis
4. Dashboard Creation in Power BI
5. Business Insights & Recommendations

---

## Dashboard
The Power BI dashboard provides interactive visualizations for:
- Revenue trends
- Customer segmentation
- Product performance
- Subscription analysis
- Purchase behavior insights

---

## Business Recommendations
- Improve customer loyalty programs
- Promote subscription benefits
- Focus marketing on high-revenue customer groups
- Optimize discount strategies
- Highlight top-performing products in campaigns

---

## Project Files
- `customer_behaviour.ipynb` → Python analysis notebook
- `customer_analysis.sql` → SQL business queries
- `dashboard_customerAnalysis.pbix` → Power BI dashboard
- `Customer Shopping Behavior Analysis.pdf` → Project report

---
